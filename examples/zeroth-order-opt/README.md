...
This project implements **zeroth-order optimization** (also known as gradient-free or derivative-free optimization) for fine-tuning large language models in GGUF format. Unlike standard fine-tuning methods that rely on backpropagation to compute gradients, this approach estimates gradients using only forward passes.

### Key Features

- ✅ **No Backpropagation**: Uses only forward passes (`llama_decode`)
- ✅ **Gradient-Free**: Estimates gradients via finite differences
- ✅ **Lower Memory**: No need to store gradients or intermediate activations
...
Simplified loss using logit magnitudes (placeholder):

```cpp
float loss_base = 0.0f;
for (int v = 0; v < std::min(100, n_vocab); ++v) {
    loss_base += logits[v] * logits[v];
}
loss_base = std::sqrt(loss_base);
```

**Note**: In production, this should be cross-entropy loss against target tokens.

#### 3. Parameter Update Loop

For each batch, randomly sample parameters and update:
...
for (int32_t p = 0; p < n_params_per_iter; ++p) {
    ...
    for (int32_t e = 0; e < n_elements_per_param; ++e) {
        ...
        ggml_backend_tensor_set(param, &perturbed_val, ...);
        float loss_perturbed = compute_loss_with_forward_pass();
...
To prevent position overflow in the context window:

```cpp
int current_pos = 0;
llama_memory_clear(llama_get_memory(ctx), true);  // Clear at epoch start

for (each batch) {
    if (current_pos + n_batch > n_ctx) {
        llama_memory_clear(llama_get_memory(ctx), true);  // Reset KV cache
        current_pos = 0;
    }
    
    // Set positions within context window
    batch.pos[j] = current_pos + j;
    current_pos += n_batch;
}
```
...
struct zeroth_order_params {
    float epsilon = 1e-2f;              // Perturbation size
    float learning_rate = 1e-1f;        // Learning rate (~100x smaller than SGD)
    float weight_decay = 0.1f;          // L2 regularization
    int32_t n_params_per_iter = 10;     // Parameters to sample per iteration
    int32_t n_elements_per_param = 5;   // Elements to update per parameter
    int32_t max_train_tokens = 6500;    // Dataset size limit (-1 for full)
};
```
...
1. **Simplified Loss**: Uses L2 norm of logits instead of cross-entropy
2. **No Full Forward Pass**: Gradient estimation is placeholder (doesn't re-decode)
3. **Random Sampling**: Parameters updated randomly, not systematically
4. **Single Perturbation**: Uses forward difference (could use central difference)
5. **No Validation Loop**: Doesn't evaluate on held-out set during training

### Theoretical Limitations

1. **Slow Convergence**: Requires many more iterations than gradient descent
2. **High Variance**: Gradient estimates are noisy
3. **Sample Inefficiency**: Needs O(d) evaluations per parameter (d = dimension)
